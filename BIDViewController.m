...
- (void)viewDidLoad
{
    ...
    div = [UIButton buttonWithType:UIButtonTypeRoundedRect];
     [div addTarget:self
       action:@selector(calculationPressed:)
     forControlEvents:UIControlEventTouchUpInside];
   ...
   // [textViewA1 setFont:[UIFont fontWithName:@"Enriqueta" size:15]];
    ...
    div1 = [UIButton buttonWithType:UIButtonTypeRoundedRect];
     [div1 addTarget:self
       action:@selector(usingNumberPadAdd:)
     forControlEvents:UIControlEventTouchUpInside];
   ...
   result1 = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    ...
    float lastValue = [[NSUserDefaults standardUserDefaults] floatForKey:@"lastResult"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    if (lastValue >0) {
        textViewA1.text=[NSString stringWithFormat:@"%.3f",lastValue];
...
- (void)usingNumberPadAdd:(id)sender {
    self.firstNumber = [textViewA1.text floatValue];
...
- (void)equalsPressed {
    self.typingNumber = NO;
    self.secondNumber = [textViewA1.text floatValue];
    
    float resultfinal = 0;
    
    if ([self.operation isEqualToString:@"+"]) {
        resultfinal = self.firstNumber + self.secondNumber;
    }
    ...
    textViewA1.text = [NSString stringWithFormat:@"%.3f", resultfinal];
    
    [[NSUserDefaults standardUserDefaults] setFloat:resultfinal forKey:@"lastResult"];
